...
<body>
	...
	<h2 class="text" style="text-align: center; margin-bottom: 8px;">Adicione uma imagem da sua loja, destaque-a</h2>
...
<?php
	if($image!=null){
		echo "<form action='' method='POST'
			...
			<input type='submit' name='dsubmit' class = 'button' style='width: 100px;  padding: 6px; font-size: 100%; margin-bottom:0; ' value='Delete isso' > ou 
			<input type='submit' class='button' name='button1' style='width: 180px;  padding: 6px; font-size: 100%;' value='Vá para a loja'/></h3>
	</form>";
}
	if($image==null){
...
if(isset($_POST['dsubmit'])){
	...
	$stmt->execute();
	if(!$stmt->rowCount()){
	myAlert("Ocorreu algum erro interno");
	header("refresh:1; url= addshopimagef.php");
	exit();
}else
	myAlert("Imagem excluída com sucesso");
	
	if(!unlink($image)){
		myAlert("Não excluído do banco de dados");
	}else{
...
$(function(){
    ...
    $('.upload-form').submit(function(e){
        if(fileInput.get(0).files.length){
            ...
            alert('Por favor escolha a imagem');
            return false;
        }
